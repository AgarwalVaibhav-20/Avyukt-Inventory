...
import { Receipt, Download, Loader2, DollarSign, TrendingUp, FileText, Search, ChevronLeft, ChevronRight } from 'lucide-react';
...
const GstTaxReportView: React.FC = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const totalTax = res.reduce((acc, curr) => acc + (curr.sgst || 0) + (curr.cgst || 0) + (curr.igst || 0), 0);
      const totalTaxable = res.reduce((acc, curr) => acc + (curr.taxableAmount || 0), 0);
      const totalInvoiceAmount = res.reduce((acc, curr) => acc + (curr.totalAmount || 0), 0);
    ...
    <div className="space-y-8 pb-8 animate-fade-in">
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-600">Monitor GST compliance and tax calculations for your business</p>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div className="group bg-white p-6 rounded-2xl border border-slate-200 shadow-md hover:shadow-xl transition-all">
                <div className="flex items-start justify-between mb-4">
                    <div>
                        <p className="text-xs font-semibold uppercase tracking-widest text-slate-500">Total Invoices</p>
                        <h3 className="text-4xl font-bold text-slate-900 mt-2">{stats.invoiceCount}</h3>
                    </div>
            ...
            <div className="group bg-white p-6 rounded-2xl border border-slate-200 shadow-md hover:shadow-xl transition-all">
                <div className="flex items-start justify-between mb-4">
                    <div>
                        <p className="text-xs font-semibold uppercase tracking-widest text-indigo-500">Taxable Amount</p>
                        <h3 className="text-4xl font-bold text-slate-900 mt-2">₹{(stats.totalTaxable / 1000).toFixed(1)}K</h3>
                    </div>
            ...
            <div className="group bg-white p-6 rounded-2xl border border-slate-200 shadow-md hover:shadow-xl transition-all">
                <div className="flex items-start justify-between mb-4">
                    <div>
                        <p className="text-xs font-semibold uppercase tracking-widest text-purple-500">Total Tax liability</p>
                        <h3 className="text-4xl font-bold text-purple-600 mt-2">₹{(stats.totalTax / 1000).toFixed(1)}K</h3>
                    </div>
            ...
            <div className="group bg-white p-6 rounded-2xl border border-slate-200 shadow-md hover:shadow-xl transition-all">
                <div className="flex items-start justify-between mb-4">
                    <div>
                        <p className="text-xs font-semibold uppercase tracking-widest text-emerald-500">Total Invoice Amount</p>
                        <h3 className="text-4xl font-bold text-emerald-600 mt-2">₹{(stats.totalInvoiceAmount / 1000).toFixed(1)}K</h3>
                    </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-8 shadow-lg">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h3 className="text-xl font-bold text-slate-900">Tax Breakdown by Type</h3>
              </div>
              <div className="h-72">
          ...
          <div className="bg-gradient-to-br from-slate-900 via-slate-800 to-blue-900 rounded-2xl p-8 text-white shadow-xl relative overflow-hidden">
              ...
              <h3 className="text-xl font-bold mb-8 flex items-center gap-2 relative z-10">
                  <Receipt className="text-yellow-400" size={24}/> GST Liability Summary
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 relative z-10">
                  <div className="bg-white/5 hover:bg-white/10 transition-colors border border-white/10 rounded-xl p-5 backdrop-blur-md">
                      <p className="text-blue-200 text-xs font-semibold uppercase tracking-wider mb-2">SGST (State GST)</p>
                      <h4 className="text-3xl font-bold">₹{stats.totalSGST.toFixed(2)}</h4>
                  </div>
                  <div className="bg-white/5 hover:bg-white/10 transition-colors border border-white/10 rounded-xl p-5 backdrop-blur-md">
                      <p className="text-blue-200 text-xs font-semibold uppercase tracking-wider mb-2">CGST (Central GST)</p>
                      <h4 className="text-3xl font-bold">₹{stats.totalCGST.toFixed(2)}</h4>
                  </div>
                  <div className="bg-white/5 hover:bg-white/10 transition-colors border border-white/10 rounded-xl p-5 backdrop-blur-md">
                      <p className="text-blue-200 text-xs font-semibold uppercase tracking-wider mb-2">IGST (Integrated GST)</p>
                      <h4 className="text-3xl font-bold">₹{stats.totalIGST.toFixed(2)}</h4>
                  </div>
                  <div className="bg-blue-500/20 border border-blue-400/30 rounded-xl p-5 backdrop-blur-md">
                      <p className="text-blue-100 text-xs font-semibold uppercase tracking-wider mb-2">Total Tax Payable</p>
              ...
              <div className="mt-8 p-4 bg-yellow-400/10 border border-yellow-400/20 rounded-xl relative z-10">
                <p className="text-sm text-yellow-100 italic font-medium flex items-center gap-2">
                  <TrendingUp size={16}/> Total invoice value including tax: ₹{stats.totalInvoiceAmount.toLocaleString()}
                </p>
              </div>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white shadow-lg overflow-hidden">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        <h3 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
                            <Receipt className="text-slate-700" size={24}/> GST Sales Register
                        </h3>
                        <p className="text-sm text-slate-600 mt-1">Detailed transaction history with tax breakdown</p>
                    </div>
                ...
                <table className="w-full text-sm text-left">
                    <thead>
                        <tr className="bg-gradient-to-r from-slate-100 to-slate-50 border-b border-slate-200">
                            ...
                            <th className="px-6 py-4 font-semibold text-slate-700 text-right">Taxable Amt</th>
                            <th className="px-6 py-4 font-semibold text-slate-700 text-right">CGST</th>
                            <th className="px-6 py-4 font-semibold text-slate-700 text-right">SGST</th>
                            <th className="px-6 py-4 font-semibold text-slate-700 text-right">IGST</th>
                            <th className="px-6 py-4 font-semibold text-slate-700 text-right">Total</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
                         {currentItems.map((row, idx) => (
                            <tr key={idx} className={`transition-colors hover:bg-blue-50 ${idx % 2 === 0 ? 'bg-white' : 'bg-slate-50'}`}>
                                <td className="px-6 py-4 font-bold text-slate-900 font-mono text-xs">{row.invoiceNo}</td>
                                <td className="px-6 py-4 text-slate-600">{row.date}</td>
                                <td className="px-6 py-4 font-semibold text-slate-800">{row.customerName}</td>
                                <td className="px-6 py-4 text-right text-slate-700 font-medium">₹{row.taxableAmount.toLocaleString(undefined, {minimumFractionDigits: 2})}</td>
                                <td className="px-6 py-4 text-right text-slate-600">₹{row.cgst.toLocaleString(undefined, {minimumFractionDigits: 2})}</td>
                                <td className="px-6 py-4 text-right text-slate-600">₹{row.sgst.toLocaleString(undefined, {minimumFractionDigits: 2})}</td>
                                <td className="px-6 py-4 text-right text-slate-600">₹{row.igst.toLocaleString(undefined, {minimumFractionDigits: 2})}</td>
                                <td className="px-6 py-4 text-right font-bold text-blue-700">₹{row.totalAmount.toLocaleString(undefined, {minimumFractionDigits: 2})}</td>
                            </tr>
                        ))}
            ...
            <div className="bg-slate-50 px-8 py-4 border-t border-slate-200 flex items-center justify-between">
                <div className="text-sm text-slate-600 font-medium">
                    Showing <span className="text-slate-900">{filtered.length > 0 ? indexOfFirstItem + 1 : 0}</span> to <span className="text-slate-900">{Math.min(indexOfLastItem, filtered.length)}</span> of <span className="text-slate-900">{filtered.length}</span> records
                </div>
